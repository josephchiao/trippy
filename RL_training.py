...
class RL_trainer:
    ...
    def train(self, variance = 0):
        ...
        for episode in range(10000):
            ...
            random_angle = np.pi - np.random.uniform(-max_wobble, max_wobble)
            
            self.model.state = [0, random_angle, 0, 0]
            ...
            while not done:
                ...
                done = next_state[1] <= np.pi/2 or next_state[1] >= 3*np.pi/2 or t >= (self.model.t_end - self.model.t_start) * self.model.fps or abs(next_state[2]) > 100 or abs(next_state[3]) > 100 or abs(next_state[0]) > 5                
                ...
                if len(states_memory) >= batch_size:
                    ...
                    self.log_std = np.clip(self.log_std, -2, 4.0)
            ...
            print(f"Episode {episode} finished! Total Reward: {total_episode_reward:.2f}, runtime = {t}")
            if t == (self.model.t_end - self.model.t_start) * self.model.fps:
                print('full runtime')

            if total_episode_reward >= best_reward + 50:
                best_reward = total_episode_reward
                self.NN.theta_save()
                previously_saved = True
                print('Saved!')

            if total_episode_reward < max(70, best_reward * 0.2):
                fail_count += 1
            else:
                fail_count = 0

            if fail_count >= 100 and previously_saved:
                self.NN.theta_recover() 
